...
class BP_MEC_Frontend {
	...
	private function access_deny() {
		...
		wp_die(__('Access Deny! You Cannot Access to Event. Please Contact to group admin.', 'mec-buddyboss'));
	}
	...
	public function meta_box_bp_group($post) {
		...
		<div class="mec-meta-box-fields mec-event-tab-content" id="mec-bp">
			...
			if( !current_user_can( 'administrator' ) ){
				...
				$g_args['user_id'] = bp_loggedin_user_id();
			}
			$groups = groups_get_groups( $g_args );

			//  groups_get_groups(array('creator_id' => bp_loggedin_user_id()));
		$groups = isset($groups['groups']) && count($groups['groups']) > 0 ? $groups['groups'] : null;
		if ($groups != null):
		?>
	...
	public function setup_nav_group() {
		...
		if (bp_mec_is_group_setup($current_group->id)) {
			$sub_nav[] = array(
	...
	public static function current_user_can_submit_event( $user_id = 0 ){
		...
		if( count( $intersect ) > 0 && !in_array( $user_id, (array)$user_ids, false ) ){
