        ...
        for($list_day = 1; $list_day <= $days_in_month; $list_day++)
        {
            ...
            if(isset($events[$today]) and count($events[$today]))
            {
                ...
                foreach($events[$today] as $event)
                {
                    ...
                    echo '</a>';
                    echo '<div id="mec_attendees_'.$event->ID.'_'.$occurrence.'_count" class="mec-admin-calendar-attendees-count">'.sprintf(esc_html__('%s attendees', 'mec'), $attendees_count).'</div>';
