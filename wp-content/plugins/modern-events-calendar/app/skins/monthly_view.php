...
class MEC_skin_monthly_view extends MEC_skins
{
    ...
    public function load_month()
    {
        ...
        do
        {
            ...
            else
            {
                ...
                if(!$this->activate_current_day and $this->month != current_time('m')) $this->active_day = '';
            }
    ...
    public function day_label($time)
    {
        ...
        {
            return '<h6 class="mec-table-side-title">'.sprintf(esc_html__('Events for %s %s', 'mec'), '<span class="mec-color mec-table-side-day"> '.esc_html($date_suffix).'</span>', esc_html($this->main->date_i18n('F', $time))).'</h6>';
        }
        else return '<h6 class="mec-table-side-title">'.sprintf(esc_html__('Events for %s', 'mec'), esc_html($this->main->date_i18n('F', $time))).'</h6><h3 class="mec-color mec-table-side-day"> '.esc_html($date_suffix).'</h3>';
    }
}