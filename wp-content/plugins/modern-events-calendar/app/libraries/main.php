...
class MEC_main extends MEC_base
{
    ...
    {
        $week_start = $this->get_first_day_of_week();
        $raw = array(array(7, esc_html__('Sunday', 'mec')), array(1, esc_html__('Monday', 'mec')), array(2, esc_html__('Tuesday', 'mec')), array(3, esc_html__('Wednesday', 'mec')), array(4, esc_html__('Thursday', 'mec')), array(5, esc_html__('Friday', 'mec')), array(6, esc_html__('Saturday', 'mec')));
            ...
            <?php if($this->getPRO()): ?>
                <li class="wns-be-group-menu-li mec-settings-menu <?php echo ($active_menu == 'booking' ? 'active' : ''); ?>">
                    <a href="<?php echo esc_url($this->add_qs_var('tab', 'MEC-booking')); ?>" id="" class="wns-be-group-tab-link-a">
                        ...
                        <span class="wns-be-group-menu-title"><?php echo esc_html($this->m('booking', esc_html__('Booking', 'mec'))); ?></span>
                    </a>
    ...
    public function booking_invoice()
    {
        ...
        {
            $settings = $this->get_settings();
            if(isset($settings['booking_invoice']) and !$settings['booking_invoice']) wp_die(__('Cannot find the invoice!', 'mec'), esc_html__('Invoice is invalid.', 'mec'));
            ...
            if(!$mec_confirmed and (!current_user_can('administrator') and !current_user_can('editor'))) wp_die(__('Your booking still is not confirmed. You can download it after confirmation!', 'mec'), esc_html__('Booking Not Confirmed.', 'mec'));
            if(!$event_id) wp_die(__('Cannot find the booking!', 'mec'), esc_html__('Booking is invalid.', 'mec'));

            // Invoice Key
            $invoice_key = isset($transaction['invoice_key']) ? $transaction['invoice_key'] : NULL;
            if($invoice_key and (!isset($_GET['mec-key']) or sanitize_text_field($_GET['mec-key']) != $invoice_key)) wp_die(__("You don't have access to view this invoice!", 'mec'), esc_html__('Key is invalid.', 'mec'));
            ...
            if(count($all_dates))
            {
                ...
                foreach($all_dates as $one_date)
                {
                    ...
                    {
                        $pdf->Write(6, sprintf(esc_html__('%s to %s', 'mec'), $this->date_i18n($date_format.' '.$time_format, $other_timestamps[0]), $this->date_i18n($date_format.' '.$time_format, $other_timestamps[1])));
                        $pdf->Ln();
                    }
                }
            ...
            {
                $pdf->SetFont('DejaVuBold', '', 12);
                $pdf->Write(6, esc_html__('Date and Time', 'mec').': ');
                $pdf->SetFont('DejaVu', '', 12);
            ...
            if(is_array($bfixed_fields) and count($bfixed_fields) and isset($transaction['fields']) and is_array($transaction['fields']) and count($transaction['fields']))
            {
                ...
                $pdf->Write(20, sprintf(esc_html__('%s Fields', 'mec'), $this->m('booking', esc_html__('Booking', 'mec'))));
                $pdf->Ln();
            ...
            {
                $pdf->SetFont('DejaVuBold', '', 16);
                $pdf->Write(20, esc_html__('Attendees', 'mec'));
                $pdf->Ln();
    ...
    public function ical_email()
    {
        ...
        if(isset($_GET['method']) and sanitize_text_field($_GET['method']) == 'ical-email')
        {
            ...
            if($key != md5($book_id)) wp_die(__('Request is not valid.', 'mec'), esc_html__('iCal export stopped!', 'mec'), array('back_link'=>true));
    ...
    public function ical_single($event_id, $occurrence = '', $occurrence_time = '', $start_from_beginning = false)
    {
        ...
        if($event->post->post_type!="mec-events"){
            return;
        }
    ...
    */
    ...
    {
        $shortcodes = [];
        $shortcodes['mce_title'] =  esc_html__('M.E. Calender', 'mec');
    ...
    {
        $start_date = isset($event->meta['mec_date']['start']) ? $event->meta['mec_date']['start'] : [];
        $end_date = isset($event->meta['mec_date']['end']) ? $event->meta['mec_date']['end'] : [];
            ...
            $messages = array(
                'taxonomies'=>array(
                    ...
                    'messages'=>array(
                        ...
                        'taxonomy_sponsors'=>array('label'=>__('Sponsor Plural Label', 'mec'), 'default'=>__('Sponsors', 'mec')),
                        'taxonomy_sponsor'=>array('label'=>__('Sponsor Singular Label', 'mec'), 'default'=>__('Sponsor', 'mec')),
                    )
                ),
                'weekdays'=>array(
                ...
                'others'=>array(
                    ...
                    'messages'=>array(
                        ...
                        'expired'=>array('label'=>__('Expired', 'mec'), 'default'=>__('Expired', 'mec')),
                        'ongoing'=>array('label'=>__('Ongoing', 'mec'), 'default'=>__('Ongoing', 'mec')),
                    )
                ),
            );
        }
    ...
    public function get_confirmation_label($confirmed = 1)
    {
        ...
        elseif($confirmed == '-1') $label = esc_html__('Rejected', 'mec');
    ...
    public function get_verification_label($verified = 1)
    {
        ...
        elseif($verified == '-1') $label = esc_html__('Canceled', 'mec');
    ...
    {
        $categories = array_merge(array(array('slug' => 'mec.block.category', 'title' => esc_html__('M.E. Calender', 'mec'), 'icon' => 'calendar-alt')), $categories);
        return $categories;
    }
    ...
    public function advanced_repeating_sort_day($start_of_week = 1, $day = 1)
    {
        ...
        array(__('Sun', 'mec'), esc_html__('Mon', 'mec'), esc_html__('Tue', 'mec'), esc_html__('Wed', 'mec'), esc_html__('Thu', 'mec'), esc_html__('Fri', 'mec'), esc_html__('Sat', 'mec'));
    ...
    public function get_normal_labels($event, $display_label = false)
    {
        ...
        // Expired Event
        elseif($display_label and $this->is_expired($event)) $output .= '<span data-style="Normal" class="mec-label-normal mec-expired-normal-label">' . $this->m('expired', esc_html__('Expired', 'mec')) . '</span>';
        // Upcoming Event
    ...
    public function timepicker($args)
    {
        ...
        if($method == 24)
        {
            ...
            ?>
            <select name="<?php echo esc_attr($name); ?>[<?php echo esc_attr($hour_key); ?>]" <?php if(trim($id_key)): ?>id="mec_<?php echo esc_attr($id_key); ?>hour"<?php endif; ?> title="<?php esc_attr_e('Hours', 'mec'); ?>">
                <?php for ($i = 0; $i <= 23; $i++) : ?>
            ...
            <select name="<?php echo esc_attr($name); ?>[<?php echo esc_attr($minutes_key); ?>]" <?php if(trim($id_key)): ?>id="mec_<?php echo esc_attr($id_key); ?>minutes"<?php endif; ?> title="<?php esc_attr_e('Minutes', 'mec'); ?>">
                <?php for ($i = 0; $i <= 11; $i++) : ?>
        ...
        else
        {
            ...
            <select name="<?php echo esc_attr($name); ?>[<?php echo esc_attr($ampm_key); ?>]" <?php if(trim($id_key)): ?>id="mec_<?php echo esc_attr($id_key); ?>ampm"<?php endif; ?> title="<?php esc_attr_e('AM / PM', 'mec'); ?>">
                <option <?php echo ($time_ampm == 'AM') ? 'selected="selected"' : ''; ?> value="AM"><?php esc_html_e('AM', 'mec'); ?></option>
                <option <?php echo ($time_ampm == 'PM') ? 'selected="selected"' : ''; ?> value="PM"><?php esc_html_e('PM', 'mec'); ?></option>
            </select>
            <?php
        }
    }
    ...
    public function holding_status($event)
    {
        ...
        elseif($this->is_expired($event)) return '<dl><dd><span class="mec-holding-status mec-holding-status-expired">'.$this->m('expired', esc_html__('Expired!', 'mec')).'</span></dd></dl>';
    ...
    {
        return $this->getDB()
            ->select("SELECT * FROM `#__mec_booking_attendees` AS a LEFT JOIN `#__mec_bookings` AS b ON a.`mec_booking_id` = b.`id` WHERE a.`id`='".esc_sql($attendee_id)."'", 'loadObject');
    }
    ...
    public function get_attendees_table($attendees, $event_id, $occurrence, $checkbox = true)
    {
        ...
        foreach($attendees as $attendee)
        {
            ...
            if($occurrence && isset($settings['certificate_status']) && $settings['certificate_status'] && isset($attendee['id']) && isset($attendee['book_id']))
            {
                ...
                $mec_attendee_id = $db->select("SELECT `id` FROM `#__mec_booking_attendees` WHERE `mec_booking_id`='".esc_sql($mec_booking_id)."' AND `ticket_id`='".esc_sql($attendee['id'])."'", "loadResult");
            }
