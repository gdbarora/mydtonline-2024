...
class MEC_notifications extends MEC_base
{
    ...
    public function email_verification($book_id, $mode = 'auto')
    {
        ...
        $subject = isset($this->notif_settings['email_verification']['subject']) ? esc_html__($this->notif_settings['email_verification']['subject'], 'mec') : esc_html__('Please verify your email.', 'mec');
        $subject = $this->content($this->get_subject($subject, 'email_verification', $event_id, $book_id), $book_id);

        $headers = array('Content-Type: text/html; charset=UTF-8');

        $recipients_str = $this->notif_settings['email_verification']['recipients'] ?? '';
        $recipients = trim($recipients_str) ? explode(',', $recipients_str) : [];

        $users = $this->notif_settings['email_verification']['receiver_users'] ?? [];
        $users_down = $this->main->get_emails_by_users($users);
        $recipients = array_merge($users_down, $recipients);

        $roles = $this->notif_settings['email_verification']['receiver_roles'] ?? [];
        ...
        foreach($attendees as $attendee)
        {
            ...
            $message = $this->notif_settings['email_verification']['content'] ?? '';
    ...
    public function booking_notification($book_id)
    {
        ...
        $subject = isset($this->notif_settings['booking_notification']['subject']) ? esc_html__($this->notif_settings['booking_notification']['subject'], 'mec') : esc_html__('Your booking is received.', 'mec');
        $subject = $this->content($this->get_subject($subject, 'booking_notification', $event_id, $book_id), $book_id);

        $headers = array('Content-Type: text/html; charset=UTF-8');

        $recipients_str = $this->notif_settings['booking_notification']['recipients'] ?? '';
        $recipients = trim($recipients_str) ? explode(',', $recipients_str) : [];

        $users = $this->notif_settings['booking_notification']['receiver_users'] ?? [];
        $users_down = $this->main->get_emails_by_users($users);
        $recipients = array_merge($users_down, $recipients);

        $roles = $this->notif_settings['booking_notification']['receiver_roles'] ?? [];
        $user_roles = $this->main->get_emails_by_roles($roles);
        $recipients = array_merge($user_roles, $recipients);
        ...
        foreach($attendees as $attendee)
        {
            ...
            $message = $this->notif_settings['booking_notification']['content'] ?? '';
    ...
    public function booking_confirmation($book_id, $mode = 'manually')
    {
        ...
        $subject = isset($this->notif_settings['booking_confirmation']['subject']) ? esc_html__($this->notif_settings['booking_confirmation']['subject'], 'mec') : esc_html__('Your booking is confirmed.', 'mec');
        $subject = $this->content($this->get_subject($subject, 'booking_confirmation', $event_id, $book_id), $book_id);

        $headers = array('Content-Type: text/html; charset=UTF-8');

        $recipients_str = $this->notif_settings['booking_confirmation']['recipients'] ?? '';
        $recipients = trim($recipients_str) ? explode(',', $recipients_str) : [];

        $users = $this->notif_settings['booking_confirmation']['receiver_users'] ?? [];
        $users_down = $this->main->get_emails_by_users($users);
        $recipients = array_merge($users_down, $recipients);

        $roles = $this->notif_settings['booking_confirmation']['receiver_roles'] ?? [];
        $user_roles = $this->main->get_emails_by_roles($roles);
        $recipients = array_merge($user_roles, $recipients);
        ...
        // Attendees
        $attendees = get_post_meta($book_id, 'mec_attendees', true);
        if(!is_array($attendees) || !count($attendees)) $attendees = array(get_post_meta($book_id, 'mec_attendee', true));
        ...
        foreach($attendees as $attendee)
        {
            ...
            $message = $this->notif_settings['booking_confirmation']['content'] ?? '';
    ...
    public function booking_cancellation($book_id)
    {
        ...
        $recipients_str = $this->notif_settings['cancellation_notification']['recipients'] ?? '';
        $recipients = trim($recipients_str) ? explode(',', $recipients_str) : [];

        $users = $this->notif_settings['cancellation_notification']['receiver_users'] ?? [];
        $users_down = $this->main->get_emails_by_users($users);
        $recipients = array_merge($users_down, $recipients);

        $roles = $this->notif_settings['cancellation_notification']['receiver_roles'] ?? [];
        $user_roles = $this->main->get_emails_by_roles($roles);
        $recipients = array_merge($user_roles, $recipients);
        ...
        $subject = isset($this->notif_settings['cancellation_notification']['subject']) ? esc_html__($this->notif_settings['cancellation_notification']['subject'], 'mec') : esc_html__('booking canceled.', 'mec');
        ...
        foreach($tos as $to)
        {
            ...
            $message = $this->notif_settings['cancellation_notification']['content'] ?? '';
    ...
    public function booking_rejection($book_id)
    {
        ...
        $recipients_str = $this->notif_settings['booking_rejection']['recipients'] ?? '';
        $recipients = trim($recipients_str) ? explode(',', $recipients_str) : [];

        $users = $this->notif_settings['booking_rejection']['receiver_users'] ?? [];
        $users_down = $this->main->get_emails_by_users($users);
        $recipients = array_merge($users_down, $recipients);

        $roles = $this->notif_settings['booking_rejection']['receiver_roles'] ?? [];
        $user_roles = $this->main->get_emails_by_roles($roles);
        $recipients = array_merge($user_roles, $recipients);
        ...
        $subject = isset($this->notif_settings['booking_rejection']['subject']) ? esc_html__($this->notif_settings['booking_rejection']['subject'], 'mec') : esc_html__('booking rejected.', 'mec');
        ...
        foreach($tos as $to)
        {
            ...
            $message = $this->notif_settings['booking_rejection']['content'] ?? '';
    ...
    public function admin_notification($book_id)
    {
        ...
        $subject = isset($this->notif_settings['admin_notification']['subject']) ? esc_html__($this->notif_settings['admin_notification']['subject'], 'mec') : esc_html__('A new booking is received.', 'mec');
        $subject = $this->content($this->get_subject($subject, 'admin_notification', $event_id, $book_id), $book_id);

        $headers = array('Content-Type: text/html; charset=UTF-8');

        $recipients_str = $this->notif_settings['admin_notification']['recipients'] ?? '';
        $recipients = trim($recipients_str) ? explode(',', $recipients_str) : [];

        $users = $this->notif_settings['admin_notification']['receiver_users'] ?? [];
        $users_down = $this->main->get_emails_by_users($users);
        $recipients = array_merge($users_down, $recipients);

        $roles = $this->notif_settings['admin_notification']['receiver_roles'] ?? [];
        $user_roles = $this->main->get_emails_by_roles($roles);
        $recipients = array_merge($user_roles, $recipients);
        ...
        $message = $this->notif_settings['admin_notification']['content'] ?? '';
    ...
    public function booking_reminder($book_id, $timestamps = NULL)
    {
        ...
        $subject = isset($this->notif_settings['booking_reminder']['subject']) ? esc_html__($this->notif_settings['booking_reminder']['subject'], 'mec') : esc_html__('Booking Reminder', 'mec');
        $subject = $this->content($this->get_subject($subject, 'booking_reminder', $event_id, $book_id), $book_id);

        $headers = array('Content-Type: text/html; charset=UTF-8');

        $recipients_str = $this->notif_settings['booking_reminder']['recipients'] ?? '';
        $recipients = trim($recipients_str) ? explode(',', $recipients_str) : [];

        $users = $this->notif_settings['booking_reminder']['receiver_users'] ?? [];
        $users_down = $this->main->get_emails_by_users($users);
        $recipients = array_merge($users_down, $recipients);

        $roles = $this->notif_settings['booking_reminder']['receiver_roles'] ?? [];
        $user_roles = $this->main->get_emails_by_roles($roles);
        $recipients = array_merge($user_roles, $recipients);
        ...
        foreach($attendees as $attendee)
        {
            ...
            $message = $this->notif_settings['booking_reminder']['content'] ?? '';
    ...
    public function new_event($event_id, $update = false)
    {
        ...
        $subject = (isset($this->notif_settings['new_event']['subject']) and trim($this->notif_settings['new_event']['subject'])) ? esc_html__($this->notif_settings['new_event']['subject'], 'mec') : esc_html__('A new event is added.', 'mec');
    ...
    */
    ...
    public function event_soldout($event_id, $book_id)
    {
        ...
        $subject = isset($this->notif_settings['event_soldout']['subject']) ? esc_html__($this->notif_settings['event_soldout']['subject'], 'mec') : esc_html__('Event is soldout!', 'mec');
    ...
    public function event_finished($event_id, $timestamps)
    {
        ...
        foreach($attendees as $attendee)
        {
            ...
            $subject = isset($this->notif_settings['event_finished']['subject']) ? esc_html__($this->notif_settings['event_finished']['subject'], 'mec') : esc_html__('Thanks for your attention!', 'mec');
    ...
    public function certificate_send($booking_attendee_id, $template)
    {
        ...
        // Subject
        $subject = isset($this->notif_settings['certificate_send']['subject']) ? esc_html__($this->notif_settings['certificate_send']['subject'], 'mec') : esc_html__('Download your certificate.', 'mec');
        ...
        // Message
        $message = isset($this->notif_settings['certificate_send']['content']) ? esc_html__($this->notif_settings['certificate_send']['content'], 'mec') : '';
    ...
    public function content($message, $book_id, $attendee = array(), $timestamps = '')
    {
        ...
        // First for All
        $first_for_all = $transaction['first_for_all'] ?? 0;
        ...
        if(trim($timestamps) and strpos($timestamps, ':') !== false)
        {
            ...
            {
                $book_datetime = sprintf(esc_html__('%s to %s', 'mec'), $this->main->date_i18n($date_format.((!$allday and !$hide_time) ? ' '.$time_format : ''), $start_timestamp), $this->main->date_i18n($date_format.((!$allday and !$hide_time and !$hide_end_time) ? ' '.$time_format : ''), $end_timestamp));
            }
        ...
        if($private_description_status)
        {
            if(is_array($attendee) and isset($attendee['id']) and !$first_for_all)
            {
                ...
                $ticket_private_description = $attendee_ticket['private_description'] ?? '';
    ...
    public function get_full_attendees_info($book_id)
    {
        ...
        foreach($attendees as $key => $attendee)
        {
            ...
            $attendees_full_info .= esc_html__('Name', 'mec').': '.((isset($attendee['name']) and trim($attendee['name'])) ? $attendee['name'] : '---')."\r\n";
        ...
        if(count($attachments))
        {
            $attendees_full_info .= esc_html__($attachment_field['label'], 'mec').': <a href="'.esc_url($attachments[0]['url']).'" target="_blank">'.esc_url($attachments[0]['url']).'</a>'."\r\n";
        }
