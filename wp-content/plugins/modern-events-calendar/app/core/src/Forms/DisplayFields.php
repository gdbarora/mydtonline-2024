			...
			<div class="mec-field-<?php echo esc_attr( $field['type'] ); ?> <?php echo $required ? 'mec-reg-mandatory' : ''; ?> <?php echo esc_attr( $class ); ?>" data-field-id="<?php echo esc_attr( $j ); ?>">
				...
				switch ( $type ) {
					case 'first_name':
						$field_type     = 'text';
						$field_id       = 'first_name';
						$field['label'] = isset( $field['label'] ) ? $field['label'] : esc_html__('First Name','mec');
						$value      = $current_user->first_name;
						break;
					case 'last_name':
						$field_type     = 'text';
						$field_id       = 'last_name';
						$field['label'] = isset( $field['label'] ) ? $field['label'] : esc_html__('Last Name','mec');
						$value      = $current_user->last_name;
						break;
					case 'mec_email':
						$field_type     = 'email';
						$field_id       = $type;
						$field['label'] = isset( $field['label'] ) ? $field['label'] : esc_html__('Email','mec');
				...
				switch ( $type ) {
					...
					case 'select':
						...
						foreach ( $options as $field_option ) {
							$rd++;
							$option_text  = isset( $field_option['label'] ) ? esc_html__( $field_option['label'], 'mec' ) : '';
					...
					case 'checkbox':
						$options = isset($field['options']) ? $field['options'] : [];
						foreach ( $options as $field_option ) {
							$current_value = esc_html__( $field_option['label'], 'mec' );
							...
							$input_html .= '<label for="' . esc_attr($html_id . $j . '_' . strtolower( str_replace( ' ', '_', $field_option['label'] ) )) . '">'
								 ...
								 . esc_html__( $field_option['label'], 'mec' )
								 . '</label>';
						}
