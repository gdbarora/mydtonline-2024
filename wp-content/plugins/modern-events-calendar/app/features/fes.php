...
class MEC_feature_fes extends MEC_base
{
    ...
    public function current_user_can_upsert_event($post_id)
    {
        ...
        if($original_post_id == $post_id && current_user_can('edit_post', $post)) return true;
        if(isset($post->post_author) and (int) $post->post_author === get_current_user_id()) return true;
    ...
    public function vlist($atts = array())
    {
        ...
        elseif($post_id == -1 or ($post_id > 0 and $this->current_user_can_upsert_event($post_id)))
        {
    ...
    public function fes_upload()
    {
        ...
        $allowed = array('gif', 'jpeg', 'jpg', 'png', 'webp');
    ...
    public function fes_form()
    {
        ...
        // Show a warning to current user if modification of post is not possible for him/her
        if(!$this->current_user_can_upsert_event($post_id)) $this->main->response(array('success'=>0, 'message'=>esc_html__("Sorry! You don't have access to modify this event.", 'mec'), 'code'=>'NO_ACCESS'));

        // Validate Captcha
        if($this->getCaptcha()->status('fes') and !$this->getCaptcha()->is_valid())
        {
            $this->main->response(array('success'=>0, 'message'=>__('Invalid Captcha! Please try again.', 'mec'), 'code'=>'CAPTCHA_IS_INVALID'));
        }
        ...
        if($is_required_content && !trim($post_content)) $this->main->response(array('success'=>0, 'message'=>__('Please fill event body field!', 'mec'), 'code'=>'BODY_IS_EMPTY'));
        ...
        if($is_required_excerpt && !trim($post_excerpt)) $this->main->response(array('success'=>0, 'message'=>__('Please fill event excerpt field!', 'mec'), 'code'=>'EXCERPT_IS_EMPTY'));
        ...
        if($is_required_dates)
        {
            ...
            if(!$start_date_is_filled) $this->main->response(array('success'=>0, 'message'=>__('Please fill event start date!', 'mec'), 'code'=>'START_DATE_IS_EMPTY'));
            if(!$end_date_is_filled) $this->main->response(array('success'=>0, 'message'=>__('Please fill event end date!', 'mec'), 'code'=>'END_DATE_IS_EMPTY'));
        }
        ...
        if($is_required_category and is_array($post_categories) and !count($post_categories)) $this->main->response(array('success'=>0, 'message'=>__('Please select at-least one category!', 'mec'), 'code'=>'CATEGORY_IS_EMPTY'));
        ...
        if($is_required_location)
        {
            ...
            if(!$location_id_is_filled and !$location_add_request) $this->main->response(array('success'=>0, 'message'=>__('Please select the event location!', 'mec'), 'code'=>'LOCATION_IS_EMPTY'));
        }
        ...
        if($is_required_label and !count($post_labels)) $this->main->response(array('success'=>0, 'message'=>__('Please select at-least one label!', 'mec'), 'code'=>'LABEL_IS_EMPTY'));
        ...
        if($is_required_featured_image and !trim($featured_image)) $this->main->response(array('success'=>0, 'message'=>__('Please upload a featured image!', 'mec'), 'code'=>'FEATURED_IMAGE_IS_EMPTY'));
        ...
        if($is_required_event_link and !trim($read_more)) $this->main->response(array('success'=>0, 'message'=>__('Please fill event link!', 'mec'), 'code'=>'EVENT_LINK_IS_EMPTY'));
        ...
        if($is_required_more_info and !trim($more_info)) $this->main->response(array('success'=>0, 'message'=>__('Please fill more info!', 'mec'), 'code'=>'MORE_INFO_IS_EMPTY'));
        ...
        if($is_required_cost and trim($cost) === '') $this->main->response(array('success'=>0, 'message'=>__('Please fill cost!', 'mec'), 'code'=>'COST_IS_EMPTY'));
        ...
        if(!isset($this->settings['fes_section_organizer']) or (isset($this->settings['fes_section_organizer']) and $this->settings['fes_section_organizer']))
        {
            ...
            $additional_organizer_ids = isset($mec['additional_organizer_ids']) ? $mec['additional_organizer_ids'] : [];
            ...
            $additional_location_ids = isset($mec['additional_location_ids']) ? $mec['additional_location_ids'] : [];
        ...
        // Repeat Options
        $repeat = isset($date['repeat']) ? $date['repeat'] : [];
        $certain_weekdays = isset($repeat['certain_weekdays']) ? $repeat['certain_weekdays'] : [];
        ...
        if(!isset($this->settings['fes_section_hourly_schedule']) or (isset($this->settings['fes_section_hourly_schedule']) and $this->settings['fes_section_hourly_schedule']))
        {
            ...
            $raw_hourly_schedules = isset($mec['hourly_schedules']) ? $mec['hourly_schedules'] : [];
        ...
        if(!isset($this->settings['fes_section_booking']) or (isset($this->settings['fes_section_booking']) and $this->settings['fes_section_booking']))
        {
            ...
            $booking = isset($mec['booking']) ? $mec['booking'] : [];
            ...
            {
                $tickets = isset($mec['tickets']) ? $mec['tickets'] : [];
                unset($tickets[':i:']);
            ...
            if(!isset($this->settings['fes_section_ticket_variations']) or (isset($this->settings['fes_section_ticket_variations']) and $this->settings['fes_section_ticket_variations']))
            {
                ...
                $ticket_variations = isset($mec['ticket_variations']) ? $mec['ticket_variations'] : [];
            ...
            if(!isset($this->settings['fes_section_reg_form']) or (isset($this->settings['fes_section_reg_form']) and $this->settings['fes_section_reg_form']))
            {
                ...
                $reg_fields = isset($mec['reg_fields']) ? $mec['reg_fields'] : [];
                if($reg_fields_global_inheritance) $reg_fields = [];

                update_post_meta($post_id, 'mec_reg_fields', $reg_fields);

                $bfixed_fields = isset($mec['bfixed_fields']) ? $mec['bfixed_fields'] : [];
