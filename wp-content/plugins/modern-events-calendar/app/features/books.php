...
class MEC_feature_books extends MEC_base
{
    ...
    ?>
        <div class="mec-book-details">
            ...
            <?php foreach($attendees as $key => $attendee): $reg_form = isset($attendee['reg']) ? $attendee['reg'] : []; ?>
            <?php
    ...
    public function save_book($post_id)
    {
        ...
        if($is_new_booking and isset($_POST['mec_attendee']) and isset($_POST['mec_date']))
        {
            ...
            // Register Attendee
            $attendees = (isset($_POST['mec_attendee']) and is_array($_POST['mec_attendee'])) ? $this->main->sanitize_deep_array($_POST['mec_attendee']) : [];
            ...
            for($i = 0; $i < $attendees_count; $i++)
            {
                ...
                $tickets[] = array_merge($attendee, array(
                    'id'=>$ticket_id,
                    'count'=>1,
                    'variations'=>(isset($attendee['variations']) ? $attendee['variations'] : array()),
        ...
        if($is_edit_booking)
        {
            ...
            {
                $original = isset($mec_attendees[$key]) ? $mec_attendees[$key] : [];
            ...
            // Coupon
            $existing_coupon = (isset($transaction['coupon']) ? $transaction['coupon'] : NULL);
    ...
    public function book()
    {
            ...
            foreach($files['name'] as $key => $value)
            {
                foreach($value as $vid => $data)
                {
                    foreach($files['name'][$key][$vid]['reg'] as $id => $reg)
                    {
                        ...
                        {
                            $file = array(
                                'name'     => $files['name'][$key][$vid]['reg'][$id],
        ...
        switch($step)
        {
            case '1':
                ...
                {
                    if(is_array($all_dates) and count($all_dates)) $total_tickets *= count($all_dates);
            ...
            case '2':
                ...
                if($first_for_all)
                {
                    ...
                    foreach($tickets as $ticket)
                    {
                        ...
                        $ticket['variations'] = isset($first_attendee['variations']) ? $first_attendee['variations'] : [];
                ...
                // MEC Cart
                $mec_cart = (isset($this->settings['mec_cart_status']) and $this->settings['mec_cart_status']) ? true : false;
                if($mec_cart) $WC_status = false;

                // Tickets
                $event_tickets = isset($event->data->tickets) ? $event->data->tickets : [];
