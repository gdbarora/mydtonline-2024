...
class MEC_feature_mec extends MEC_base
{
    ...
    public function activate_license()
    {
        if($this->getPRO())
        {
            ...
            if($verify && isset($verify->item_link))
            {
                ...
                $payload['status'] = true;
                $payload['button_text'] = esc_html__('revoke' , 'mec');
            ...
            else
            {
                ...
                $payload['button_text'] = esc_html__('submit' , 'mec');
    ...
    public function revoke_license()
    {
        if($this->getPRO())
        {
            ...
            $payload = json_encode(array('message'=> 'revoked' , 'status'=> true, 'button_text' => esc_html__('submit' , 'mec')));
            echo MEC_kses::element($payload);
            wp_die();
        }
    }
    ...
    {
        add_submenu_page('mec-intro', esc_html__('MEC - Support', 'mec'), esc_html__('Support', 'mec'), 'manage_options', 'MEC-support', array($this, 'support_page'));
    }
    ...
    public function menus()
    {
        ...
        add_submenu_page('mec-intro', esc_html__('Add Event', 'mec'), esc_html__('Add Event', 'mec'), apply_filters( 'mec_menu_cap', 'edit_posts', 'add_event' ), 'post-new.php?post_type='.$this->PT);
        add_submenu_page('mec-intro', esc_html__('Tags', 'mec'), esc_html__('Tags', 'mec'), apply_filters( 'mec_menu_cap', 'edit_others_posts', 'tags' ), 'edit-tags.php?taxonomy='.apply_filters('mec_taxonomy_tag', '').'&post_type='.$this->PT);
        add_submenu_page('mec-intro', esc_html($this->main->m('taxonomy_categories', esc_html__('Categories', 'mec'))), esc_html($this->main->m('taxonomy_categories', esc_html__('Categories', 'mec'))), apply_filters( 'mec_menu_cap', 'edit_others_posts', 'categories' ), 'edit-tags.php?taxonomy=mec_category&post_type='.$this->PT);
        add_submenu_page('mec-intro', esc_html($this->main->m('taxonomy_labels', esc_html__('Labels', 'mec'))), esc_html($this->main->m('taxonomy_labels', esc_html__('Labels', 'mec'))), apply_filters( 'mec_menu_cap', 'edit_others_posts', 'labels' ), 'edit-tags.php?taxonomy=mec_label&post_type='.$this->PT);
        add_submenu_page('mec-intro', esc_html($this->main->m('taxonomy_locations', esc_html__('Locations', 'mec'))), esc_html($this->main->m('taxonomy_locations', esc_html__('Locations', 'mec'))), apply_filters( 'mec_menu_cap', 'edit_others_posts', 'locations' ), 'edit-tags.php?taxonomy=mec_location&post_type='.$this->PT);

        // Organizers Menu
        if(!isset($this->settings['organizers_status']) || $this->settings['organizers_status'])
        {
            add_submenu_page('mec-intro', esc_html($this->main->m('taxonomy_organizers', esc_html__('Organizers', 'mec'))), esc_html($this->main->m('taxonomy_organizers', esc_html__('Organizers', 'mec'))), apply_filters( 'mec_menu_cap', 'edit_others_posts', 'organizers' ), 'edit-tags.php?taxonomy=mec_organizer&post_type='.$this->PT);
        }

        // Speakers Menu
        if(isset($this->settings['speakers_status']) and $this->settings['speakers_status'])
        {
            add_submenu_page('mec-intro', esc_html($this->main->m('taxonomy_speakers', esc_html__('Speakers', 'mec'))), esc_html($this->main->m('taxonomy_speakers', esc_html__('Speakers', 'mec'))), apply_filters( 'mec_menu_cap', 'edit_others_posts', 'speakers' ), 'edit-tags.php?taxonomy=mec_speaker&post_type='.$this->PT);
        }

        // Sponsors Menu
        if($this->getPRO() and isset($this->settings['sponsors_status']) and $this->settings['sponsors_status'])
        {
            add_submenu_page('mec-intro', esc_html($this->main->m('taxonomy_sponsors', esc_html__('Sponsors', 'mec'))), esc_html($this->main->m('taxonomy_sponsors', esc_html__('Sponsors', 'mec'))), apply_filters( 'mec_menu_cap', 'edit_others_posts', 'sponsors' ), 'edit-tags.php?taxonomy=mec_sponsor&post_type='.$this->PT);
        }
        ...
        add_submenu_page('mec-intro', esc_html__('Shortcodes', 'mec'), esc_html__('Shortcodes', 'mec'), $capability, 'edit.php?post_type=mec_calendars');
        ...
        if($this->getPRO() and isset($this->settings['auto_emails_module_status']) and $this->settings['auto_emails_module_status'])
        {
            ...
            add_submenu_page('mec-intro', esc_html__('Emails', 'mec'), esc_html__('Emails', 'mec'), $capability, 'edit.php?post_type=mec-emails');
        }
        ...
        if($this->getPRO() and isset($this->settings['certificate_status']) and $this->settings['certificate_status'])
        {
            ...
            add_submenu_page('mec-intro', esc_html__('Certificates', 'mec'), esc_html__('Certificates', 'mec'), $capability, 'edit.php?post_type='.$this->main->get_certificate_post_type());
        }
        ...
        if(isset($this->settings['webhooks_status']) and $this->settings['webhooks_status'])
        {
            ...
            add_submenu_page('mec-intro', esc_html__('Webhooks', 'mec'), esc_html__('Webhooks', 'mec'), $capability, 'edit.php?post_type=mec-webhooks');
        }

        $capability = (current_user_can('administrator') ? 'manage_options' : 'mec_settings');
        add_submenu_page('mec-intro', esc_html__('MEC - Settings', 'mec'), esc_html__('Settings', 'mec'), apply_filters( 'mec_menu_cap', $capability, 'settings' ), 'MEC-settings', array($this, 'page'));

        add_submenu_page('mec-intro', esc_html__('MEC - Addons', 'mec'), esc_html__('Addons', 'mec'), apply_filters( 'mec_menu_cap', 'manage_options', 'addons' ), 'MEC-addons', array($this, 'addons'));
        add_submenu_page('mec-intro', esc_html__('MEC - Wizard', 'mec'), esc_html__('Wizard', 'mec'), apply_filters( 'mec_menu_cap', 'manage_options', 'wizard' ), 'MEC-wizard', array($this, 'setup_wizard'));

        if(isset($this->settings['booking_status']) and $this->settings['booking_status'])
        {
            add_submenu_page('mec-intro', esc_html__('MEC - Report', 'mec'), esc_html__('Report', 'mec'), 'mec_report', 'MEC-report', array($this, 'report'));
        }

        if(!$this->getPRO()) add_submenu_page('mec-intro', esc_html__('MEC - Go Pro', 'mec'), esc_html__('Go Pro', 'mec'), apply_filters( 'mec_menu_cap', 'manage_options', 'go_pro' ), 'MEC-go-pro', array($this, 'go_pro'));
    ...
    {
        $columns['shortcode'] = esc_html__('Shortcode', 'mec');
        return $columns;
    }
    ...
    public function register_meta_boxes()
    {
		...
		add_meta_box('mec_calendar_display_options', esc_html__('Display Options', 'mec'), array($this, 'meta_box_display_options'), 'mec_calendars', 'normal', 'high');
        add_meta_box('mec_calendar_filter', esc_html__('Filter Options', 'mec'), array($this, 'meta_box_filter'), 'mec_calendars', 'normal', 'high');
        add_meta_box('mec_calendar_shortcode', esc_html__('Shortcode', 'mec'), array($this, 'meta_box_shortcode'), 'mec_calendars', 'side');
        add_meta_box('mec_calendar_search_form', esc_html__('Search Form', 'mec'), array($this, 'meta_box_search_form'), 'mec_calendars', 'side');
        add_meta_box('mec_calendar_icons', esc_html__('Icon Options', 'mec'), [$this, 'meta_box_icons'], 'mec_calendars', 'normal', 'low');
    }
    ...
    public function action_links($actions, $post)
    {
        ...
        {
            // Duplicate Button
            $actions['mec-duplicate'] = '<a href="'.esc_url($this->main->add_qs_vars(array('mec-action'=>'duplicate-shortcode', 'id'=>$post->ID))).'">'.esc_html__('Duplicate', 'mec').'</a>';
        }
    ...
    public function sed_method_field($skin, $value = 0, $image_popup = 0)
    {
        $image_popup_html = '<div class="mec-form-row mec-image-popup-wrap mec-switcher">
            <div class="mec-col-4">
                <label for="mec_skin_'.esc_attr($skin).'_image_popup">'.esc_html__('Display content\'s images as Popup', 'mec').'</label>
            </div>
            <div class="mec-col-4">
        ...
        return '<div class="mec-form-row mec-sed-method-wrap">
            <div class="mec-col-4">
                <label for="mec_skin_'.esc_attr($skin).'_sed_method">'.esc_html__('Single Event Display Method', 'mec').'</label>
            </div>
            <div class="mec-col-8">
                ...
                <ul class="mec-sed-methods" data-for="#mec_skin_'.esc_attr($skin).'_sed_method_field">
                    <li data-method="0" class="'.(!$value ? 'active' : '').'"><a href="#mec_skin_'.esc_attr($skin).'_sed_method_field">'.esc_html__('Current Window', 'mec').'</a></li>
                    <li data-method="new" class="'.($value === 'new' ? 'active' : '').'"><a href="#mec_skin_'.esc_attr($skin).'_sed_method_field">'.esc_html__('New Window', 'mec').'</a></li>
                    <li data-method="m1" class="'.($value === 'm1' ? 'active' : '').'"><a href="#mec_skin_'.esc_attr($skin).'_sed_method_field">'.esc_html__('Modal Popup', 'mec').'</a></li>
                    <li data-method="no" class="'.($value === 'no' ? 'active' : '').'"><a href="#mec_skin_'.esc_attr($skin).'_sed_method_field">'.esc_html__('Disable Link', 'mec').'</a></li>
                </ul>
            </div>
    ...
    public function order_method_field($skin, $value = 'ASC', $key = 'order_method')
    {
        return '<div class="mec-form-row mec-switcher">
            <div class="mec-col-4">
                <label for="mec_skin_'.esc_attr($skin).'_'.esc_attr($key).'">'.esc_html__('Descending Order', 'mec').'</label>
            </div>
            <div class="mec-col-4">
                ...
                <span class="mec-tooltip">
                    <div class="box right">
                        <h5 class="title">'.esc_html__('Descending Order', 'mec').'</h5>
                        <div class="content"><p>'.esc_html__('Enable this option to sort events in descending order.', 'mec').'</p></div>
                    </div>
    ...
    public function booking_button_field($skin, $value = 0)
    {
        ...
        return '<div class="mec-form-row mec-booking-button-wrap mec-switcher mec-not-list-liquid mec-not-grid-liquid mec-not-daily_view-liquid mec-not-full_calendar-liquid">
            <div class="mec-col-4">
                <label for="mec_skin_'.esc_attr($skin).'_booking_button">'.esc_html__('Booking Button / Icon', 'mec').'</label>
            </div>
            <div class="mec-col-4">
                ...
                <span class="mec-tooltip">
                    <div class="box right">
                        <h5 class="title">'.esc_html__('Booking Button / Icon', 'mec').'</h5>
                        <div class="content"><p>'.esc_html__('Enable this option to display booking button / icon in this shortcode.', 'mec').'</p></div>
                    </div>
                    <i title="" class="dashicons-before dashicons-editor-help"></i>
                </span>
            </div>
    ...
    public function display_organizer_field($skin, $value = 0)
    {
        return '<div class="mec-form-row mec-display-organizer-wrap mec-switcher mec-not-grid-liquid mec-not-daily_view-liquid">
            <div class="mec-col-4">
                <label for="mec_skin_'.esc_attr($skin).'_display_organizer">'.esc_html__('Display Organizers', 'mec').'</label>
            </div>
            <div class="mec-col-4">
                ...
                <span class="mec-tooltip">
                    <div class="box right">
                        <h5 class="title">'.esc_html__('Display Organizers', 'mec').'</h5>
                        <div class="content"><p>'.esc_html__('Enable this option to display events organizers in this shortcode.', 'mec').'</p></div>
                    </div>
                    <i title="" class="dashicons-before dashicons-editor-help"></i>
                </span>
            </div>
    ...
    public function display_custom_data_field($skin, $value = 0)
    {
        return '<div class="mec-form-row mec-display-organizer-wrap mec-switcher mec-not-list-liquid mec-not-grid-liquid mec-not-daily_view-liquid mec-not-full_calendar-liquid">
            <div class="mec-col-4">
                <label for="mec_skin_'.esc_attr($skin).'_custom_data">'.esc_html__('Display Custom Fields', 'mec').'</label>
            </div>
            <div class="mec-col-4">
                ...
                <span class="mec-tooltip">
                    <div class="box right">
                        <h5 class="title">'.esc_html__('Display Custom Fields', 'mec').'</h5>
                        <div class="content"><p>'.esc_html__('Enable this option to display events custom data in this shortcode.', 'mec').'</p></div>
                    </div>
                    <i title="" class="dashicons-before dashicons-editor-help"></i>
                </span>
            </div>
    ...
    public function display_pagination_field($skin, $options = [])
    {
        ...
        return '<div class="mec-form-row mec-event-pagination-container mec-switcher mec-not-'.esc_attr($skin).'-liquid">
            <div class="mec-col-4">
                <label for="mec_skin_'.esc_attr($skin).'_pagination">'.esc_html__('Pagination Method', 'mec').'</label>
            </div>
            <div class="mec-col-4">
                <select name="mec[sk-options]['.esc_attr($skin).'][pagination]" id="mec_skin_'.esc_attr($skin).'_pagination">
                    <option value="loadmore" '.($value === 'loadmore' ? 'selected' : '').'>'.esc_html__('Load More Button', 'mec').'</option>
                    <option value="scroll" '.($value === 'scroll' ? 'selected' : '').'>'.esc_html__('Infinite Scroll', 'mec').'</option>
                    <option value="nextprev" '.($value === 'nextprev' ? 'selected' : '').'>'.esc_html__('Next / Prev','mec').'</option>
                    <option value="0" '.($value === '0' ? 'selected' : '').'>'.esc_html__('Disabled', 'mec').'</option>
                </select>
            </div>
        </div>';
    }
    ...
    {
        return ($skin === 'full_calendar' ? '<p class="description mec-not-full_calendar-liquid">'.esc_html__("The price shows in List View and Tile View.", 'mec').'</p>' : '').'
        <div class="mec-form-row mec-event-price-container mec-switcher mec-not-'.esc_attr($skin).'-liquid">
            <div class="mec-col-4">
                <label for="mec_skin_'.esc_attr($skin).'_display_price">'.esc_html__('Display Event Price', 'mec').'</label>
            </div>
            <div class="mec-col-4">
                ...
                <span class="mec-tooltip">
                    <div class="box right">
                        <h5 class="title">'.esc_html__('Display Event Price', 'mec').'</h5>
                        <div class="content"><p>'.esc_html__('Enable this option to display events price in this shortcode.', 'mec').'</p></div>
                    </div>
                    <i title="" class="dashicons-before dashicons-editor-help"></i>
                </span>
            </div>
    ...
    public function display_status_bar_field($skin, $value = 0)
    {
        return '<div class="mec-form-row mec-switcher mec-status-bar-display-wrapper mec-util-hidden" id="mec_skin_'.esc_attr($skin).'_display_status_bar_field">
            <div class="mec-col-4">
                <label for="mec_skin_'.esc_attr($skin).'_status_bar">'.esc_html__('Display Status Bar', 'mec').'</label>
            </div>
            <div class="mec-col-4">
                ...
                <span class="mec-tooltip">
                    <div class="box right">
                        <h5 class="title">'.esc_html__('Display Organizers', 'mec').'</h5>
                        <div class="content"><p>'.esc_html__('Enable this option to display the status bar in this shortcode.', 'mec').'</p></div>
                    </div>
                    <i title="" class="dashicons-before dashicons-editor-help"></i>
                </span>
            </div>
    ...
    public function display_progress_bar_field($skin, $value)
    {
        ...
        return '<div class="mec-form-row mec-switcher mec-progress-bar-display-wrapper mec-util-hidden" id="mec_skin_'.esc_attr($skin).'_display_progress_bar_field">
            <div class="mec-col-4">
                <label for="mec_skin_'.esc_attr($skin).'_display_progress_bar">'.esc_html__('Display Progress Bar (Ongoing Events)', 'mec').'</label>
            </div>
            <div class="mec-col-4">
    ...
    public function display_detailed_time_field($skin, $value = 0)
    {
        return '<div class="mec-form-row mec-switcher mec-not-daily_view-liquid">
            <div class="mec-col-4">
                <label for="mec_skin_'.esc_attr($skin).'_detailed_time">'.esc_html__('Detailed Time', 'mec').'</label>
                <span>'.esc_html__('For Multiple Day Events', 'mec').'</span>
            </div>
            <div class="mec-col-4">
    ...
    public function dashboard_widgets()
    {
        wp_add_dashboard_widget(
            ...
            esc_html__('Modern Events Calendar', 'mec'),
    ...
    public function widget_news()
    {
        ...
        echo '<div class="mec-metabox-head-wrap">
            <div class="mec-metabox-head-version">
                ...
                <p>'.($this->getPRO() ? esc_html__('Modern Events Calendar', 'mec') : esc_html__('Modern Events Calendar (Lite)', 'mec')).'</p>
            ...
            jQuery(document).ready(function()
            {
                ...
                var mecSellsChart = new Chart(ctx,
                {
                    ...
                    data:
                    {
                        ...
                        datasets: [
                        {
                            label: "'.esc_js(sprintf(esc_html__('Total Sales (%s)', 'mec'), $currency)).'",
                            data: ['.trim($stats, ', ').'],
                    ...
                    <div class="w-box-content">
                        <ul>
                            ...
                            <li class="mec-chart-last-year"><a href="<?php echo esc_url(add_query_arg(array(
                                ...
                                'type' => 'monthly',
                            ))); ?>"><?php esc_html_e('Last Year', 'mec'); ?></a></li>
                        </ul>
                        <script>
                        </script>
                        <?php
                        ...
                        <form class="mec-sells-filter" method="GET" action="">
                            ...
                            <input type="text" class="mec_date_picker" name="start" placeholder="<?php esc_attr_e('Start Date', 'mec'); ?>" value="<?php echo esc_attr($start); ?>" />
                            <input type="text" class="mec_date_picker" name="end" placeholder="<?php esc_attr_e('End Date', 'mec'); ?>" value="<?php echo esc_attr($end); ?>" />
                            <select name="type">
                                <option value="daily" <?php echo ($type == 'daily' ? 'selected="selected"' : ''); ?>><?php esc_html_e('Daily', 'mec'); ?></option>
                                <option value="monthly" <?php echo ($type == 'monthly' ? 'selected="selected"' : ''); ?>><?php esc_html_e('Monthly', 'mec'); ?></option>
                                <option value="yearly" <?php echo ($type == 'yearly' ? 'selected="selected"' : ''); ?>><?php esc_html_e('Yearly', 'mec'); ?></option>
                            </select>
                            <select name="chart">
                                <option value="bar" <?php echo ($chart == 'bar' ? 'selected="selected"' : ''); ?>><?php esc_html_e('Bar', 'mec'); ?></option>
                                <option value="line" <?php echo ($chart == 'line' ? 'selected="selected"' : ''); ?>><?php esc_html_e('Line', 'mec'); ?></option>
                            </select>
                            <button type="submit"><?php esc_html_e('Filter', 'mec'); ?></button>
                        </form>
                ...
                <div class="w-box total-bookings print-events">
                    ...
                    <div class="w-box-content">
                        <form method="GET" action="<?php echo home_url(); ?>" target="_blank">
                            ...
                            <button type="submit"><?php esc_html_e('Display Events', 'mec'); ?></button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    public function register_assets_per_page_meta_boxes()
    {
        ...
        add_meta_box('mec_metabox_app', esc_html__('Include MEC Assets', 'mec'), array($this, 'meta_box_assets_per_page'), $post_types, 'side', 'low');
    }
    ...
    public function meta_box_assets_per_page($post)
    {
        ...
        <div class="mec-assets-per-page-metabox">
            <label for="mec_include_assets">
                ...
                <?php esc_html_e('Include Modern Events Calendar Assets (CSS, JavaScript, etc files.)', 'mec'); ?>
            </label>
        </div>
        <?php
    }
    ...
    public function dropdown()
    {
        ...
        $success = 0;
        $html = '<option class="mec-load-occurrences" value="'.esc_attr($_6months_ago.':'.$_6months_ago).'">'.esc_html__('Previous Occurrences', 'mec').'</option>';
        ...
        if(count($occurrences) >= $limit and isset($occurrence)) $html .= '<option class="mec-load-occurrences" value="'.esc_attr($occurrence->tstart.':'.$occurrence->tend).'">'.esc_html__('Next Occurrences', 'mec').'</option>';
    ...
    public function search_form_event_fields($skin, $options = array())
    {
        ...
        foreach($event_fields as $field_id => $event_field)
        {
            ...
            <div class="mec-form-row">
                ...
                <select class="mec-col-12" name="mec[sf-options][<?php echo esc_attr($skin); ?>][fields][<?php echo esc_attr($field_id); ?>][type]" id="mec_sf_<?php echo esc_attr($skin); ?>_fields_<?php echo esc_attr($field_id); ?>">
                    <option value="0" <?php if(isset($field_options[$field_id]) and isset($field_options[$field_id]['type']) and $field_options[$field_id]['type'] == '0') echo 'selected="selected"'; ?>><?php esc_html_e('Disabled', 'mec'); ?></option>
                    <?php if(in_array($type, array('text', 'textarea'))): ?>
                    <option value="text_input" <?php if(isset($field_options[$field_id]) and isset($field_options[$field_id]['type']) and $field_options[$field_id]['type'] == 'text_input') echo 'selected="selected"'; ?>><?php esc_html_e('Text Input', 'mec'); ?></option>
                    <?php elseif($type === 'date'): ?>
                    <option value="date-range-picker" <?php if(isset($field_options[$field_id]) and isset($field_options[$field_id]['type']) and $field_options[$field_id]['type'] == 'date-range-picker') echo 'selected="selected"'; ?>><?php esc_html_e('Date Range Picker', 'mec'); ?></option>
                    <?php else: ?>
                    <option value="dropdown" <?php if(isset($field_options[$field_id]) and isset($field_options[$field_id]['type']) and $field_options[$field_id]['type'] == 'dropdown') echo 'selected="selected"'; ?>><?php esc_html_e('Dropdown', 'mec'); ?></option>
                    <?php endif; ?>
                </select>
            </div>
            <?php
        ...
        if(is_string($fields) and trim($fields)) $fields = '<h4 style="margin-bottom: 0;">'.esc_html__('Event Fields', 'mec').'</h4><hr style="margin-top: 3px;">' .$fields;
        return $fields;
    }
