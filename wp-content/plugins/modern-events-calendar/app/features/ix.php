...
class MEC_feature_ix extends MEC_base
{
    ...
    public function menus()
    {
        ...
        add_submenu_page('mec-intro', esc_html__('MEC - Import / Export', 'mec'), esc_html__('Import / Export', 'mec'), $capability, 'MEC-ix', array($this, 'ix'));
    }
    ...
    public function import_start_bookings()
    {
        ...
        if(!isset($feed_file['name']) or trim($feed_file['name']) == '') return array('success' => 0, 'message' => esc_html__('Please upload a CSV file.', 'mec'));

        // File name validation
        $name_ex = explode('.', $feed_file['name']);
        $name_end = end($name_ex);
        if($name_end != 'csv') return array('success' => 0, 'message' => esc_html__('Please upload a CSV file.', 'mec'));
        ...
        // Error on Upload
        if(!$uploaded) return array('success' => 0, 'message' => esc_html__("An error occurred during the file upload! Please check permissions!", 'mec'));

        if($type = mime_content_type($target_path) and $type == 'text/x-php')
        {
            unlink($target_path);
            return array('success' => 0, 'message' => esc_html__("Please upload a CSV file.", 'mec'));
        }
        ...
        if(($h = fopen($target_path, 'r')) !== false)
        {
            ...
            while(($data = fgetcsv($h, 1000, "\t")) !== false)
            {
                ...
                elseif($confirmed_label == esc_html__('Rejected', 'mec')) $confirmed = -1;
                else $confirmed = 0;

                $verified_label = $data[14];
                if($verified_label == esc_html__('Verified', 'mec')) $verified = 1;
                elseif($verified_label == esc_html__('Canceled', 'mec')) $verified = -1;
                else $verified = 0;
        ...
        return array('success' => (count($bookings) ? 1 : 0), 'message' => (count($bookings) ? esc_html__('The bookings are imported successfully!', 'mec') : esc_html__('No bookings found to import!', 'mec')));
    }
    ...
    public function import_start()
    {
        ...
        // File is not uploaded
        if(!isset($feed_file['name']) or trim($feed_file['name']) == '') return array('success' => 0, 'message' => esc_html__('Please upload the feed file.', 'mec'));

        // File name validation
        $ex = explode('.', $feed_file['name']);
        $name_end = end($ex);
        if(!in_array($name_end, array('xml', 'ics'))) return array('success' => 0, 'message' => esc_html__('Please upload an XML or an ICS file.', 'mec'));

        // File Type is not valid
        if(!isset($feed_file['type']) or !in_array(strtolower($feed_file['type']), array('text/xml', 'text/calendar'))) return array('success' => 0, 'message' => esc_html__('The file type should be XML or ICS.', 'mec'));
        ...
        {
            unlink($target_path);
            return array('success' => 0, 'message' => esc_html__("Please upload an XML or an ICS file.", 'mec'));
        }

        if($type === 'text/calendar' and is_string($this->main->parse_ics($target_path)))
        {
            return array('success' => 0, 'message' => sprintf(__("The ICS file is not valid. Reported Error: %s", 'mec'), '<strong>'.$this->main->parse_ics($target_path).'</strong>'));
        }
        ...
        return array('success' => 1, 'message' => esc_html__('The events are imported successfully!', 'mec'));
    }
    ...
    public function thirdparty_import_do()
    {
        ...
        $response = array('success'=>0, 'message'=>__('Third Party plugin is invalid!', 'mec'));
    ...
    {
        $api_key = $this->ix['meetup_api_key'] ?? NULL;
        $group_url = $this->ix['meetup_group_url'] ?? NULL;
    ...
    public function meetup_import_do()
    {
        ...
        foreach($m_events as $m_event)
        {
            ...
            // Event Organizer
            $organizers = $event->event_hosts ?? NULL;
            $main_organizer_id = 1;
            $additional_organizer_ids = [];
            ...
            // Meetup Link
            $more_info = $event->link ?? ($event->short_link ?? '');
    ...
    public function g_calendar_export_authenticate()
    {
        ...
        $this->main->response(array('success'=>1, 'message'=>sprintf(esc_html__('All seems good! Please click %s to authenticate your app.', 'mec'), '<a href="'.esc_url($auth_url).'">'.esc_html__('here', 'mec').'</a>')));
    }
