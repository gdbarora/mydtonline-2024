 ...
 */
function mec_find_wordpress_base_path_gi()
{
...
define('BASE_PATH', mec_find_wordpress_base_path_gi().'/');
...
if(!isset($internal_cron_system) && (!isset($ix['sync_g_import']) || !$ix['sync_g_import'])) exit(__('Auto-import for Google Calendar is disabled!', 'mec'));

$api_key = $ix['google_import_api_key'] ?? NULL;
$calendar_id = $ix['google_import_calendar_id'] ?? NULL;
...
try
{
    ...
    $message = (count($posts) ? sprintf(esc_html__('%s google events imported/updated.', 'mec'), count($posts)) : '');
    $message .= (count($posts_deleted) ? ' '.sprintf(esc_html__('%s events trashed successfully.', 'mec'), count($posts_deleted)) : '');
