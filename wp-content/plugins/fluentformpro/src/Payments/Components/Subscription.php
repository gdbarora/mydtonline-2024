...
class Subscription extends BaseFieldManager
{
    ...
    )
    {
        ...
        add_filter('fluentform_editor_init_element_subscription_payment_component', function ($element) {
            if (!isset($element['settings']['layout_class'])) {
                $element['settings']['layout_class'] = '';
            }
            return $element;
        });
        add_filter('fluentform_response_render_' . $this->key, function ($response, $field, $form_id, $isHtml = false) {
            if (!$isHtml) {
                return $response;
            }
    ...
    function getComponent()
    {
        return array(
            ...
            'editor_options' => array(
                'title'      => __('Subscription Field', 'fluentformpro'),
