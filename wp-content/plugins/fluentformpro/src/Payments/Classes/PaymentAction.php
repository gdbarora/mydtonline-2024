...
//            $items = apply_filters(
//                'fluentform_submitted_payment_items_' . $this->selectedPaymentMethod,
//                $items,
//                $this->form,
//                $this->data,
//                $subscriptionItems
...
//        }
        ...
        do_action('fluentform/notify_on_form_submit', $this->submissionId, $this->submissionData, $this->form);
        ...
        if ($this->selectedPaymentMethod) {
            ...
            do_action('fluentform_process_payment', $this->submissionId, $this->submissionData, $this->form, $this->methodSettings, !!$subscriptionItems, $totalPayable);
            do_action('fluentform_process_payment_' . $this->selectedPaymentMethod, $this->submissionId, $this->submissionData, $this->form, $this->methodSettings, !!$subscriptionItems, $totalPayable);
        }
    ...
    public function getOrderItems($forced = false)
    {
        ...
        $this->orderItems = apply_filters('fluentform_submission_order_items', $this->orderItems, $this->submissionData, $this->form);
    ...
    private function getItemFromVariables($item, $key)
    {
        ...
        $pricingOptions = apply_filters('fluentform_payment_field_' . $elementName . '_pricing_options', $pricingOptions, $item, $this->form);
    ...
    public function getSubscriptionItems()
    {
        ...
        $this->subscriptionItems = apply_filters('fluentform_submission_subscription_items', $this->subscriptionItems, $this->submissionData, $this->form);
    ...
    private function insertOrderItems($items, $existing = false)
    {
        ...
        foreach ($items as $item) {
            ...
            if (in_array($hash, $existingHashes)) {
                ...
                $newId = wpFluent()->table('fluentform_order_items')->insert($item);
                $verifiedIds[] = $newId;
                $newIds[] = $newId;
            }
        }
    ...
    protected function applyDiscountCodes()
    {
        ...
        foreach ($subscriptionItems as $subIndex => $subscriptionItem) {
            ...
            foreach ($this->discountCodes as $coupon) {
                $discountAmount = $coupon->amount;
                if ($coupon->coupon_type == 'percent') {
                    $discountRecurringAmount = intval((floatval($discountAmount) / 100) * $recurringAmount);
                    $recurringAmount -= $discountRecurringAmount;
                    if ($signupFee) {
                        $discountSignupDiscountAmount = intval((floatval($discountAmount) / 100) * $signupFee);
