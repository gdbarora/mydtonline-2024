...
class DraftSubmissionsManager
{
    ...
    public function saveState($hash)
    {
        ...
        if (!$exist) {
            ...
            if ($isStepForm) {
                ...
                do_action('fluentform_saved_progress_submission_added', $formData['data'], $response, $insertId,
                    $formId);
            }
        }
        else {
    ...
    public function maybeLoadSavedProgress()
    {
        ...
        add_action('fluentform_before_form_render', function () use ($key) {
            ...
            wp_localize_script('form-save-progress', 'form_state_save_vars', [
                ...
                'copy_button'         => sprintf("<img src='%s' >", $wpFluentForm->url('public/img/copy.svg')),
                'copy_success_button' => sprintf("<img src='%s' >", $wpFluentForm->url('public/img/check.svg')),
                'email_button'        => sprintf("<img src='%s' >", $wpFluentForm->url('public/img/email.svg')),
            ]);
        });
    }
    ...
    public function emailProgressLink()
    {
        ...
        $emailFormat = apply_filters('fluentform_email_form_resume_link_config', $emailFormat, $submittedData, $requestData, $form);
        do_action('fluentform_email_form_resume_link_before_sent', $emailFormat, $submittedData, $requestData, $form);
        ...
        if ($notify) {
            ...
            wp_send_json_success([
                'response' => apply_filters('fluentform_email_resume_link_response',$message)
            ]);
        }
    ...
    private function processEmail($settings, $form, $link, $toEmail)
    {
        ...
        if (false !== strpos($emailBody, '{email_resume_link}')) {
            ...
            $emailBody = apply_filters('fluentform_email_resume_link_body', $emailBody, $form, $link);
        }
    ...
    private function verify()
    {
        ...
        if (!wp_verify_nonce($nonce)) {
            $message = apply_filters('fluentform_nonce_error',
