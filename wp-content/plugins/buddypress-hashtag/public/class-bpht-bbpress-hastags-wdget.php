		...
		<div>
			...
			if ( $wpdb->get_var( "SHOW TABLES LIKE '$table_name'" ) ) {
				if ( 3 == $instance['sortby'] ) {
					...
					$sql       = "SELECT * FROM $table_name WHERE ht_type IN ('bbpress') ORDER BY ht_count DESC, ht_last_count {$sortorder} LIMIT $limit";
				} else {
					$sql = "SELECT * FROM $table_name WHERE ht_type IN ('bbpress') ORDER BY ht_count DESC LIMIT $limit";
				}
				 ...
				 */
				$query     = apply_filters( 'wbcom_hashtag_widget_query', $sql, 'bbpress', $limit );
			...
			if ( count( $hashtags ) && is_array( $hashtags ) ) {
					...
					<div class="bpht-widget--hashtags">
						<div class="bpht-hashtags-wrapper <?php echo esc_attr( $wrapper ); ?>">
							...
							foreach ( $hashtags as $name => $hash_data ) {
								...
								echo ' <a href="' . esc_url( $site_url . $forum_slug . '/' . $search_slug ) . '/%23' . esc_html( htmlspecialchars( $name ) ) . '" rel="nofollow" class="hashtag" id="' . esc_attr( htmlspecialchars( $name ) ) . '">#' . esc_html( htmlspecialchars( $name ) );
				...
				<?php } else { ?>
					<div>
						<span class='bpht-text--muted'><?php echo esc_html__( 'No hashtags', 'buddypress-hashtags' ); ?></span>
					</div>
				<?php } ?>
			</div>
			<?php
