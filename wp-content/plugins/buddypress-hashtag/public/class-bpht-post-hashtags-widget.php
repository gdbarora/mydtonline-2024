...
class BPHT_Hashtag_Post_Widget extends WP_Widget {

	public function __construct( $id = null, $name = null, $args = null ) {

		$id   = ( null !== $id ) ? $id : 'BPHT_Hashtag_Post_Widget';
		$name = ( null !== $name ) ? $name : __( 'Posts Hashtags', 'buddypress-hashtags' );
		$args = ( null !== $args ) ? $args : array( 'description' => __( 'Posts Hashtags Widget', 'buddypress-hashtags' ) );
	...
	public function widget( $args, $instance ) {
		$instance = apply_filters( 'BPHT_Hashtag_Post_Widget_instance', $instance );
		...
		<div>
			...
			if ( $wpdb->get_var( "SHOW TABLES LIKE '$table_name'" ) ) {
				if ( 3 == $instance['sortby'] ) {
					...
					$sql       = "SELECT * FROM $table_name WHERE ht_type IN ('post') ORDER BY ht_count DESC, ht_last_count {$sortorder} LIMIT $limit";
				} else {
					$sql = "SELECT * FROM $table_name WHERE ht_type IN ('post') ORDER BY ht_count DESC LIMIT $limit";
				}
				 ...
				 */
				$query     = apply_filters( 'wbcom_hashtag_widget_query', $sql, 'post', $limit );
					...
					<div class="bpht-widget--hashtags">
						<div class="bpht-hashtags-wrapper <?php echo esc_attr( $wrapper ); ?>">
							...
							foreach ( $hashtags as $name => $hash_data ) {
								...
								echo ' <a href="' . esc_url( $site_url ) . '?s=%23' . esc_html( htmlspecialchars( $name ) ) . '" rel="nofollow" class="hashtag" id="' . esc_attr( htmlspecialchars( $name ) ) . '">#' . esc_html( htmlspecialchars( $name ) );
	...
	public function form( $instance ) {
		...
		if ( ! isset( $instance['title'] ) ) {
			$instance['title'] = __( 'Posts Hashtags', 'buddypress-hashtags' );
		}
