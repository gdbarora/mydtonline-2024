...
class Buddypress_Hashtags_Admin {
	...
	public function enqueue_styles() {
		...
		if ( isset( $_GET['page'] ) && $_GET['page'] == 'buddypress_hashtags' ) { //phpcs:ignore
	...
	public function wbcom_hide_all_admin_notices_from_setting_page() {
		$wbcom_pages_array  = array( 'wbcomplugins', 'wbcom-plugins-page', 'wbcom-support-page', 'buddypress_hashtags', 'wbcom-license-page' );
	...
	public function bpht_add_menu_buddypress_hashtags() {
		...
		add_submenu_page( 'wbcomplugins', esc_html__( 'BuddyPress Hashtags Setting Page', 'buddypress-hashtags' ), esc_html__( 'BuddyPress Hashtag', 'buddypress-hashtags' ), 'manage_options', 'buddypress_hashtags', array( $this, 'bpht_hashtags_settings_page' ) );
	}
	...
	public function bpht_hashtags_settings_page() {
		...
		<div class="wrap">
				...
				<div class="blpro-header">
					<div class="wbcom_admin_header-wrapper">
						<div id="wb_admin_plugin_name">
							<?php esc_html_e( 'BuddyPress Hashtag', 'buddypress-hashtags' ); ?>
							<?php /* translators: %s: */ ?>
							<span><?php printf( esc_html__( 'Version %s', 'buddypress-hashtags' ), esc_attr( BPHT_PLUGIN_VERSION ) ); ?></span>
						</div>
			...
			$bpht_tabs = array(
				...
				'hashtag-logs' => __( 'Hashtag Index', 'buddypress-hashtags' ),
			...
			foreach ( $bpht_tabs as $bpht_tab => $bpht_name ) {
				...
				$tab_html .= '<li class=' . $bpht_name . '><a class="nav-tab ' . $class . '" href="admin.php?page=buddypress_hashtags&tab=' . $bpht_tab . '">' . $bpht_name . '</a></li>';
			}
			$tab_html .= '</div></ul></div>';
	...
	public function bpht_update_hashtags_links_on_save_post( $data, $postarr ) {
		...
		*/
		...
		if ( isset( $_REQUEST['wp_customize'] ) && $_REQUEST['wp_customize'] == 'on' ) { //phpcs:ignore
			return $data;
		}

		$allow_post_types = array( 'post', 'page', 'forum', 'topic', 'reply' );

		if ( ! empty( $data['post_content'] ) && $data['post_status'] == 'publish' && isset( $data['post_type'] ) && in_array( $data['post_type'], $allow_post_types ) ) {
			...
			if ( $hashtags ) {
				...
				foreach ( (array) $hashtags as $hashtag ) {
					...
					if ( strpos( $data['post_content'], '?s=%23' . htmlspecialchars( $hashtag ) ) != '' ) {
						continue;
					}
					...
					bpht_db_buddypress_hashtag_entry( $hashtag, $post_type, $postarr['ID'] );
				}
			}
		}
		return $data;
		...
		if ( ! empty( $commentdata['comment_content'] ) ) {
			...
			if ( $hashtags ) {
				...
				foreach ( (array) $hashtags as $hashtag ) {
					...
					if ( strpos( $commentdata['comment_content'], '?s=%23' . htmlspecialchars( $hashtag ) ) != '' ) {
						continue;
					}
					...
					bpht_db_buddypress_hashtag_entry( $hashtag, $post_type, $commentdata['comment_ID'] );
				}
			}
		}
		return $commentdata;
