...
class GamiPress_Leaderboard_Table {
    ...
    public function parse_query_vars() {
            ...
            if( in_array( $metric, $achievement_types ) ) {
                ...
                if( ! empty( $start_date ) || ! empty( $end_date ) ) {
                    ...
                    if( is_multisite() && ! gamipress_is_network_wide_active() ) {
                        // Retrieve the current amount of points from the user meta 'prefix__gamipress_{$points_type}_points'
                        $prefix_site = $wpdb->get_blog_prefix();
                        $select[] = "IFNULL( um{$i}.meta_value, 0 ) AS `{$metric}`";
                        $join[] = "LEFT JOIN {$usermeta} AS um{$i} ON ( um{$i}.user_id = u.ID AND um{$i}.meta_key = '{$prefix_site}_gamipress_{$metric}_points' )";    
                    } else if ( is_multisite() && gamipress_is_network_wide_active() ){
        ...
        if( $query_vars['items_per_page'] !== 0 && ( $query_vars['items_per_page'] < $query_vars['max_items'] || $query_vars['max_items'] === 0 ) ) {
            ...
            if ( $query_vars['max_items'] > 0 && ( ( $offset + $query_vars['items_per_page'] ) > $query_vars['max_items'] ) ) {
                $last_page = $query_vars['max_items'] - $offset;
                $query_vars['limit'] = "LIMIT {$offset}, {$last_page}";
            } else {
