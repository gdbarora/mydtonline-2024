	...
	class LearndashHelper implements BBLMSHelper {
		...
		public function ajax_get_courses() {
			...
			if ($c_q->have_posts()) {
				...
				$language_filter = sanitize_text_field($_GET["filter-languages"]);

				if ($language_filter !== 'all') {
				    // Filter the courses based on language
				    $filtered_courses = array();
				    foreach ($user_courses as $course_id) {
				        $language_terms = get_the_terms($course_id, 'ld_course_language');

				        if ($language_terms && !is_wp_error($language_terms)) {
				            $language_slugs = wp_list_pluck($language_terms, 'slug');

				            if (in_array($language_filter, $language_slugs)) {
				                $filtered_courses[] = $course_id;
				            }
				        }
				    }
				    $user_courses = $filtered_courses;
				}

				$childCategories = [];

				if ((isset($_REQUEST['filter-categories']) && ($_REQUEST['filter-categories'] == 'all'))) :	
					...
					$categories = get_term_by('slug', $_REQUEST['filter-categories'], 'ld_course_category');
				...
				if((isset($_REQUEST['filter-topics']) && ($_REQUEST['filter-topics'] != 'all'))) :
						$child_cat = get_term_by('slug', $_REQUEST['filter-topics'], 'ld_course_category');
				endif;
				$catUserCourses = [];
				foreach ($user_courses as $course_id) :
				...
				<h2><?php echo $child_cat->name; ?></h2>
<!-- 						<div class="course_category_desc py-2"><?php echo nl2br($cat->description); ?></div> -->
						<?php
						echo '<ul dev-data="learndash-helper" class="bb-course-list bb-course-items bb-grid list-view ' . ('list' != $view ? 'hide' : '') . '" aria-live="assertive" aria-relevant="all">';
						if (isset($catUserCourses[$child_cat->term_id]) && count($catUserCourses[$child_cat->term_id]) > 0 ) :
							$user_courses = $catUserCourses[$child_cat->term_id];
							foreach ($user_courses as $course_id) :
								if ($course_id !== 0) :
					...
					echo '<ul data-case="hi" class="bb-card-list bb-course-items grid-view bb-grid ' . ('grid' != $view ? 'hide' : '') . '" aria-live="assertive" aria-relevant="all">';
					if (isset($catUserCourses[$child_cat->term_id]) && count($catUserCourses[$child_cat->term_id]) > 0) :
						$user_courses = $catUserCourses[$child_cat->term_id];
						foreach ($user_courses as $course_id) :
							if ($course_id !== 0) :
...
<!-- 						<div class="course_category_desc py-2"><?php echo nl2br($cat->description); ?></div> -->
		...
		protected function _archive_filterby_tax ( $query ) {
			...
			if ( ! empty( $_GET[ "filter-languages" ] ) && 'all' != $_GET['filter-languages'] ) {
				

				$tax_query[] = array(
					'taxonomy'         => 'ld_course_language',
					'field'            => 'slug',
					'terms'            => $_GET["filter-languages"],
					'include_children' => false,
				);
			}

			if ( ! empty( $_GET[ "filter-topics" ] ) && 'all' != $_GET['filter-topics'] ) {
				

				$tax_query[] = array(
					'taxonomy'         => 'ld_course_category',
					'field'            => 'slug',
					'terms'            => $_GET["filter-topics"],
					'include_children' => false,
				);
			}
