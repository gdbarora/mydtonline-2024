...
$doc_created_date = bp_get_document_date();
$month = date('m', strtotime($doc_created_date));
$year = date('Y', strtotime($doc_created_date));
$uploads_dir = wp_upload_dir();

$document_path = $uploads_dir['baseurl']. '/bb_documents/'. $year .'/'. $month .'/'. $document_title;
$document_name = preg_replace("/[\-_]/", " ", $document_title);
?>
...
id="div-listing-<?php bp_document_id(); ?>">
	<div class="media-folder_icon">
		<a href="<?php echo esc_url( $document_folder_link ); ?>"> <i class="<?php echo esc_attr( $svg_icon ); ?>">
		<embed src="<?php echo $document_path; ?><?php echo $extension ? '.' . esc_html( $extension ) : ''; ?>" alt="File Preview" height="50px" width="40px"/></i></a>
	</div>
	<div class="media-folder_details">
		...
		data-icon-class="<?php echo esc_attr( $svg_icon ); ?>">
			<span><?php echo esc_html( $document_name ); ?></span><?php //echo $extension ? '.' . esc_html( $extension ) : ''; ?>
