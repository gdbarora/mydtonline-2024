...
<li class="lg-grid-1-5 md-grid-1-3 sm-grid-1-3" data-id="<?php bp_video_id(); ?>" data-date-created="<?php bp_video_date_created(); ?>">
	...
	if(array_search(bp_loggedin_user_id(), $group_admins) !== false){
		?>
						<li class="move_video_to_videoalbum">
								<a href="#" data-video-id="<?php bp_video_id(); ?>" data-parent-activity-id="<?php bp_video_parent_activity_id(); ?>" data-item-activity-id="<?php bp_video_activity_id(); ?>" data-type="<?php echo esc_attr( $move_type ); ?>" id="<?php echo esc_attr( $move_id ); ?>" class="dt-video-move"><?php esc_html_e( 'Move to Video Album', 'buddyboss' ); ?></a>
						</li>
						<?php
	}
						?>
