...
$register_date        = date_i18n( 'd M Y', strtotime( get_userdata( $user_id )->user_registered ) );
?>
...
<?php if ( ! bp_is_user_messages() && ! bp_is_user_settings() && ! bp_is_user_notifications() && ! bp_is_user_profile_edit() && ! bp_is_user_change_avatar() && ! bp_is_user_change_cover_image() ) : ?>
	...
	<div id="cover-image-container" class="<?php echo esc_attr( $profile_header_layout_style . ' ' . $social_networks_urls_div_class . ' ' . $my_profile ); if(bp_is_current_component('courses') && bp_is_my_profile()) : echo ' my_courses_tab_hide'; endif;?> bb-cover-image-container">
		<?php if(bp_is_current_component('courses') && bp_is_my_profile()) :?>
		<div class="course_comp_headings">
			<h2>My Courses</h2>
			<p>This page displays all courses you currently have access to. As you complete courses or rank up more will become available.
		</div><?php else: ?>
